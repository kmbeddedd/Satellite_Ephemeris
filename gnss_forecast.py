...
# ──────────────────────────────────────────────────────────────────
DATA_PATH        = "FINAL_Data.csv"   # override via --data
...
def build_model():
    ...
    x = Bidirectional(LSTM(32, return_sequences=True), name="bilstm")(inp)
    x = Dropout(0.3, name="drop1")(x)

    # Encoder — GRU compression
    x = GRU(64, return_sequences=False, name="gru")(x)
    x = Dropout(0.11, name="drop2")(x)
    ...
    model = Model(inp, out)
    model.compile(
        optimizer=Adam(learning_rate=1.56e-3),
